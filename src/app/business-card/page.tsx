                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                    <div className="bg-card border border-white/5 rounded-2xl p-6 md:p-8 glass-premium">
                        ...
                        <div className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <Input id="name" name="name" value={formData.name} onChange={handleInputChange} placeholder="John Doe" className="bg-black/20 border-white/10 focus-visible:ring-primary/50" />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="title" className="text-sm font-medium flex items-center gap-2">
                                        <User className="w-4 h-4 text-primary" /> Job Title
                                    </Label>
                                    <Input id="title" name="title" value={formData.title} onChange={handleInputChange} placeholder="Chief AI Officer" className="bg-black/20 border-white/10 focus-visible:ring-primary/50" />
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <Input id="company" name="company" value={formData.company} onChange={handleInputChange} placeholder="RootedAI Solutions" className="bg-black/20 border-white/10 focus-visible:ring-primary/50" />
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <Input id="phone" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="+91 98765 43210" className="bg-black/20 border-white/10 focus-visible:ring-primary/50" />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="email" className="text-sm font-medium flex items-center gap-2">
                                        <Mail className="w-4 h-4 text-primary" /> Email Address
                                    </Label>
                                    <Input id="email" name="email" type="email" value={formData.email} onChange={handleInputChange} placeholder="john@rootedai.co.in" className="bg-black/20 border-white/10 focus-visible:ring-primary/50" />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <Input id="website" name="website" value={formData.website} onChange={handleInputChange} placeholder="www.rootedai.co.in" className="bg-black/20 border-white/10 focus-visible:ring-primary/50" />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="address" className="text-sm font-medium flex items-center gap-2">
                                        <MapPin className="w-4 h-4 text-primary" /> Location
                                    </Label>
                                    <Input id="address" name="address" value={formData.address} onChange={handleInputChange} placeholder="Bangalore, India" className="bg-black/20 border-white/10 focus-visible:ring-primary/50" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-8 pt-6 border-t border-white/10 flex justify-end">
                            <MagneticButton>
                                <Button onClick={handleDownload} disabled={isDownloading} className="bw-button group flex items-center gap-2">
                                    {isDownloading ? <span className="animate-pulse">Generating...</span> : <><Download className="w-4 h-4 group-hover:-translate-y-1 transition-transform" /> Download High-Res Card</>}
                                </Button>
                            </MagneticButton>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center sticky top-24">
                        ...
                        <div className="relative w-full max-w-[600px] aspect-[1.75] rounded-xl overflow-hidden shadow-2xl ring-1 ring-white/10 group">
                            <div ref={cardRef} className="absolute inset-0 bg-[#050505] text-white overflow-hidden p-8 flex flex-col justify-between" style={{ width: '100%', height: '100%' }}>
                                <div className="absolute inset-0 pointer-events-none opacity-40 mix-blend-screen" style={{ backgroundImage: `radial-gradient(circle at center, rgba(160, 160, 160, 0.15) 1px, transparent 1px)`, backgroundSize: '24px 24px' }} />
                                ...
                                <div className="relative z-10 flex justify-between items-start">
                                    <div>
                                        <h2 className="font-heading font-bold text-3xl tracking-wide mb-1 text-white uppercase" style={{ wordSpacing: '4px' }}>{formData.name || 'Your Name'}</h2>
                                        <p className="font-sans text-primary font-medium tracking-wider text-sm uppercase">{formData.title || 'Your Title'}</p>
                                    </div>
                                    <div className="w-12 h-12 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center glass-premium">
                                        <span className="font-heading font-bold text-xl text-primary">{formData.company ? formData.company.charAt(0).toUpperCase() : 'R'}</span>
                                    </div>
                                </div>
                                <div className="relative z-10 flex justify-between items-end mt-auto">
                                    <div className="space-y-2.5 font-sans">
                                        {formData.phone && <div className="flex items-center gap-3 text-sm text-gray-300"><Phone className="w-4 h-4 text-primary/80" /><span className="tracking-wide">{formData.phone}</span></div>}
                                        {formData.email && <div className="flex items-center gap-3 text-sm text-gray-300"><Mail className="w-4 h-4 text-primary/80" /><span className="tracking-wide">{formData.email}</span></div>}
                                        {formData.website && <div className="flex items-center gap-3 text-sm text-gray-300"><Globe className="w-4 h-4 text-primary/80" /><span className="tracking-wide">{formData.website}</span></div>}
                                        {formData.address && <div className="flex items-center gap-3 text-sm text-gray-300 pt-1"><MapPin className="w-4 h-4 text-primary/80" /><span className="tracking-wide">{formData.address}</span></div>}
                                    </div>
                                    <div className="text-right">
                                        <p className="font-heading font-bold text-xl tracking-tight text-white mb-1">{formData.company || 'Company'}</p>
                        ...
                        <p className="text-xs text-muted-foreground mt-4 text-center max-w-[400px]">The preview above represents exactly how your card will look when exported. Resolution will be scaled up automatically for high-quality printing.</p>
                    </div>
                </div>
            </div>
        </div>
