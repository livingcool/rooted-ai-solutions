...
const Contact = () => {
  ...
  return (
    <section id="contact" className="py-16 relative overflow-hidden bg-transparent">
      ...
      <div className="absolute top-0 right-0 w-[50%] h-[100%] bg-violet-500/5 blur-[120px] rounded-full translate-x-1/2 -translate-y-1/2 pointer-events-none" />
          ...
          <div className="lg:col-span-5 space-y-8">
            <div className="space-y-6">
              <h2 className="text-5xl md:text-7xl font-black text-black dark:text-white tracking-tighter leading-tight">
                Let's <span className="bg-gradient-to-r from-violet-600 to-purple-600 bg-clip-text text-transparent italic">Talk</span>
              </h2>
            ...
            <div className="relative aspect-square max-w-[300px] mx-auto lg:mx-0 group">
              <div className="absolute inset-0 bg-violet-500/20 rounded-full blur-3xl animate-pulse group-hover:bg-violet-500/40 transition-colors" />
              <div className="relative h-full w-full glass-premium rounded-full border border-white/20 flex items-center justify-center overflow-hidden">
                ...
                {/* The Core Orb */}
                <div 
                  className={`w-32 h-32 rounded-full shadow-[0_0_50px_rgba(139,92,246,0.5)] transition-all duration-500 ${
                    Object.values(formData).some(v => v !== "") ? 'scale-125 bg-violet-400 rotate-45' : 'scale-100 bg-violet-600'
                  }`}
                >
                ...
                <div className="absolute inset-0 animate-spin-slow">
                  ...
                  <div className="absolute bottom-4 left-1/2 w-3 h-3 bg-violet-400 rounded-full blur-sm" />
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-2 group">
                    <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground group-focus-within:text-violet-500 transition-colors">Personal Alias</label>
                    <Input
                      value={formData.name}
                      onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                      required
                      className="bg-transparent border-0 border-b-2 border-white/10 rounded-none h-14 text-lg focus:border-violet-500 transition-all px-0 placeholder:text-muted-foreground/30"
                      placeholder="Your name"
                    />
                  </div>
                  <div className="space-y-2 group">
                    <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground group-focus-within:text-violet-500 transition-colors">Secure Email</label>
                    <Input
                      type="email"
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-2 group">
                    <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground group-focus-within:text-violet-500 transition-colors">Organization</label>
                    <Input
                  ...
                  <div className="space-y-2 group">
                    <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground group-focus-within:text-violet-500 transition-colors">Primary Wave (Phone)</label>
                    <Input
                      type="tel"
                ...
                <div className="space-y-2 group">
                  <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground group-focus-within:text-violet-500 transition-colors">Directive (Service)</label>
                  <select
                    value={formData.service_needed}
                    onChange={(e) => setFormData({ ...formData, service_needed: e.target.value })}
                    className="w-full bg-transparent border-0 border-b-2 border-white/10 rounded-none h-14 text-lg focus:border-violet-500 focus:outline-none transition-all px-0 appearance-none cursor-pointer"
                  >
                ...
                <div className="space-y-2 group">
                  <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground group-focus-within:text-violet-500 transition-colors">Project Brief</label>
                  <Textarea
                    value={formData.message}
                    onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                    required
                    className="bg-transparent border-0 border-b-2 border-white/10 rounded-none min-h-[120px] text-lg focus:border-violet-500 transition-all px-0 resize-none placeholder:text-muted-foreground/30"
